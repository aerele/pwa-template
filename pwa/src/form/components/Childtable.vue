<template>
	<div class="p-2">
		<div class="flex justify-between p-2">
			...
			<Button @click="newChild" :disabled="isDisabled">
		...
		<transition name="slide-up" @enter="handleEnter" @leave="handleLeave">
			...
			>
				...
				<div class="flex border-t-[1.5px] border-gray-200 p-4">
					...
					<div v-else>
						<FeatherIcon name="trash-2" class="w-6 h-6 text-red-700 mt-1 hover:cursor-pointer hover:text-red-800" />
					</div>
					<div v-if="indexValueSlected >= 0" class=" ml-auto">
						<Button variant="solid" class="w-[5rem] ml-auto" @click="update" :disabled="isDisabled">Update</Button>
					</div>
					<div v-else class=" ml-auto">
						<Button variant="solid" class="w-[5rem] ml-auto" @click="Add" :disabled="isDisabled">Add</Button>
					</div>
				</div>
			</div>
		</transition>        
...
watch(props.field, (newField) => {
	if(newField.value && retrivedValue.value){
...
const Delete = () => {
	...
	if(JSON.stringify(records.value) != JSON.stringify(props.field.value)){
		props.frm.Saved = 0;
		props.frm.Submit = 0;
		props.frm.Amend = 0;
	}
...
const filteredFields = ref(props.field.options)
</script>
<style scoped>
